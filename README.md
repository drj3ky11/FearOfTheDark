...
Los casos son totalmente independientes entre sí y no introducen conceptos nuevos — solo demuestran, sobre datos reales, lo ya explicado en los bloques teóricos.

| Caso | Contenido | Duración |
|---|---|---|
| [`HackingForums/`](HackingForums/) | Identidad y tiempo — pivoting cross-foro sobre 5 foros underground (2013–2021) | 1h 40min |
| [`ransomware/`](ransomware/) | Anatomía de una organización criminal — Conti, BlackBasta, LockBit | 1h 40min |
| [`IronMarch/`](IronMarch/) | Radicalización y red social — foro de extrema derecha con ground truth pública | 1h 40min |

Cada caso sigue la misma estructura de 5 notebooks (`00_reconocimiento` → `04_sintesis_informe`), tiene su propia presentación y su `README.md` con el detalle de datasets y hallazgos.

[`CardingForums/`](CardingForums/) sigue el mismo pipeline de 5 notebooks y es la fuente de datos que se usa en vivo durante la demo del Bloque 3 — no es un caso práctico con sesión propia.
...
**Narrativa**: IronMarch fue el principal foro de neonazis aceleracionistas en habla inglesa. Fue desmantelado en 2017 y su base de datos filtrada en 2019. Lo que hace este dataset único es que varios de sus miembros son ahora individuos **públicamente identificados** vinculados a ataques terroristas reales — lo que permite validar el análisis con ground truth conocida.
