# Enfoque de los 4 casos prácticos — para la reunión con el compañero
...
|---|---|---|
| Ransomware (compañero) | ¿Cómo se organiza jerárquicamente **un** grupo criminal? | Clasificación de roles con LLM + similitud de embeddings entre grupos (linaje) |
| IronMarch | ¿Quién tiene **influencia real** en la red? | Centralidad/brokers + validación contra ground truth judicial |
...
La distinción que sostiene esto: Ransomware mira una organización, IronMarch mira
quién manda, HackingForums mira si una persona persiste en el tiempo, Carding
mira cómo se divide el trabajo en un mercado sin jerarquía. Evita que "grafos" o
...
**Pendiente para la reunión**: preguntar si el pipeline de ransomware se ha
corrido de punta a punta en algún entorno y si los números del
`GUION_TALLER.md` (matriz de cohesión 0.914, etc.) vienen de una ejecución real
reproducible, o si aún falta correrlo.
...
Decidir en la reunión: ¿se acorta cada caso a ~1h15, o Carding se queda con un
formato más ligero (su rol actual en el README es "demo en vivo del Bloque 3",
no caso con sesión propia)?
...
**Reducir** (ya visto en bloques teóricos, no repetir en profundidad):
parsing de formatos rotos (NDJSON concatenado), limpieza, detección de idioma.
...
- **Para qué sirve la matriz de cohesión 4×4** — validar o refutar con
  evidencia semántica (no solo cronológica) la hipótesis de que BlackBasta es
  una escisión/sucesión directa de Conti.
...
### IronMarch — brokers de radicalización
...
- **Por qué degree Y betweenness**, no solo uno — degree mide alcance directo,
  ...
  hablarían entre sí. Son dos formas de poder distintas.
...
- **Para qué sirve comparar red pública (posts) vs. privada (PMs)** — este es
  el corazón pedagógico: el usuario 255 tiene 53% del betweenness en PMs pero
  es invisible en la actividad pública. Liderazgo visible ≠ liderazgo real.
...
**Reducir**: EDA comparativo, detección de idioma (bloque 2), limpieza
epoch-0 (ya visto en Carding).

**Profundizar**:
- **Por qué 4 snapshots de la misma comunidad** (OGUsers), no solo comparar
  dos foros distintos una vez — permite ver evolución tras cada brecha, no una
  foto fija.
- **Por qué ninguna señal sola basta** — handle es trivial de cambiar,
  embeddings solos no distinguen argot compartido de identidad compartida.
  Justifica el score combinado.
- **Por qué Burrows' Delta específicamente** — palabras función (no
  vocabulario de contenido) es la técnica de atribución de autoría más
  resistente a que el autor cambie de tema o de plataforma.
...
contraste**: el perfilado LLM de roles individuales (es exactamente lo que
hace Ransomware con otro vocabulario).
...
- **Por qué filtrar aristas de un solo thread compartido** antes de correr
  Louvain — sin filtrar, aparecen comunidades falsas por coincidencias
  triviales.
